...
def _record_audio(screen: curses.window) -> npt.NDArray[np.float32]:
    ...
    with sd.InputStream(samplerate=44100, channels=1, dtype=np.float32, callback=_audio_callback):
        while True:
            key = screen.getch()
            if key == ord(" "):
...
class AudioPlayer:
    def __enter__(self):
        self.stream = sd.OutputStream(samplerate=44100, channels=1, dtype=np.int16)
        self.stream.start()
        return self
